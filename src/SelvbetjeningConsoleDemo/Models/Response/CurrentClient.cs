...
public class CurrentClient
{
    ...
    /// <summary>
    /// Converts this into an update model that keeps all of its state when used to update the client
    /// </summary>
    public ClientUpdate ToClientUpdate()
    {
        return new ClientUpdate
        {
            ApiScopes = ApiScopes.Select(s => s.Scope).ToList(),
            ...
            PostLogoutRedirectUris = PostLogoutRedirectUris.Length == 0 ? null : PostLogoutRedirectUris.ToList(),
            ChildOrganizationNumbers = ChildOrganizationNumbers.ToList(),
        };
    }
}
