...
namespace SelvbetjeningConsoleDemo.Auth;
...
public sealed class UserAuthenticator : IDisposable
{
    ...
    private readonly SystemBrowserRunner _browser;

    public UserAuthenticator(UserClientData clientData, string htmlTitle, string htmlBody)
    {
        _clientData = clientData;
        _httpClient = new HttpClient();
        _browser = new SystemBrowserRunner(htmlTitle, htmlBody);
        _oidcClient = CreateOidcClient();
    }

    public async Task<(ResourceTokens, string idToken)> LoginAndGetTokens(string[]? resources = null)
    {
        ...
        var resourcesToGetTokensFor =
            resources?.Length > 0 ? resources : _clientData.Resources.Select(r => r.Name).ToArray();
        ...
        return (new ResourceTokens([.. resourceTokens], latestRefreshToken), loginResult.IdentityToken);
    }
    ...
    public async Task<ResourceTokens> GetTokens(string refreshToken, string resource)
    {
        ...
        var refreshResult =
            await _oidcClient.RefreshTokenAsync(refreshToken, new Parameters { { "resource", resource } });
    ...
    private OidcClient CreateOidcClient()
    {
        ...
        var scope = $"openid offline_access profile {apiScopes}";
        ...
        var options = new OidcClientOptions
        {
            ...
            GetClientAssertionAsync = () => Task.FromResult(ClientAssertionBuilder.Build(_clientData.ClientId,
                _clientData.Jwk.PublicAndPrivateValue, _clientData.Authority)),
            Browser = _browser,
            LoadProfile = false,
